...
public class Trigger implements BooleanSupplier {
    ...
    public Trigger(Scheduler scheduler, EventLoop loop, BooleanSupplier condition) {
        ...
        BindingScope bindingScope = BindingScope.createNarrowestScope(m_scheduler);
        if (bindingScope instanceof BindingScope.RobotModeScope) {
            m_creationScope = BindingScope.Global.INSTANCE;
        } else {
            m_creationScope = bindingScope;
        }
    ...
    public Trigger risingEdge() {
        return new Trigger(m_scheduler, m_loop, () -> m_cachedSignal == Signal.HIGH && m_previousSignal == Signal.LOW);
    }
    ...
    public Trigger fallingEdge() {
        return new Trigger(m_scheduler, m_loop, () -> m_cachedSignal == Signal.LOW && m_previousSignal == Signal.HIGH);
    }
        ...
        return new Trigger(
            ...
            new BooleanSupplier() {
                ...
                public boolean getAsBoolean() {
                    ...
                    while (!m_timestamps.isEmpty() && currentTime - m_timestamps.peekFirst() > durationSeconds + 1e-9) {
    ...
    private void cancelBindings(BindingType bindingType) {
        m_bindings.getOrDefault(bindingType, List.of()).forEach(binding -> m_scheduler.cancel(binding.command()));
    }
    ...
    private void toggleBindings(BindingType bindingType) {
        m_bindings.getOrDefault(bindingType, List.of()).forEach(binding -> {
