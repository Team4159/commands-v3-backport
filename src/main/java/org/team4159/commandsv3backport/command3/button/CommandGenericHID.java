...
package org.team4159.commandsv3backport.command3.button;
...
public final class CommandGenericHID {
    ...
    private final Map<EventLoop, Map<Pair<Integer, Double>, Trigger>> m_axisLessThanCache = new HashMap<>();
    private final Map<EventLoop, Map<Pair<Integer, Double>, Trigger>> m_axisGreaterThanCache = new HashMap<>();
    ...
    public static CommandGenericHID getCommandGenericHID(Scheduler scheduler, int port) {
        m_hidsLock.lock();
        try {
            CommandGenericHID[] hids = m_hids.computeIfAbsent(
                scheduler,
                k -> new CommandGenericHID[DriverStationBackend.JOYSTICK_PORTS]
            );
    ...
    public Trigger button(int button, EventLoop loop) {
        ...
        return cache.computeIfAbsent(button, k -> new Trigger(m_scheduler, loop, () -> m_hid.getRawButton(k)));
    }
    ...
    public Trigger pov(int pov, POVDirection angle, EventLoop loop) {
        ...
        return cache.computeIfAbsent(pov * 16 + angle.value, k ->
            new Trigger(m_scheduler, loop, () -> m_hid.getPOV(pov) == angle)
        );
    }
    ...
    public Trigger axisLessThan(int axis, double threshold, EventLoop loop) {
        ...
        return cache.computeIfAbsent(Pair.of(axis, threshold), k ->
            new Trigger(m_scheduler, loop, () -> getRawAxis(axis) < threshold)
        );
    }
    ...
    public Trigger axisMagnitudeGreaterThan(int axis, double threshold, EventLoop loop) {
        ...
        return cache.computeIfAbsent(Pair.of(axis, threshold), k ->
            new Trigger(m_scheduler, loop, () -> Math.abs(getRawAxis(axis)) > threshold)
        );
    }
