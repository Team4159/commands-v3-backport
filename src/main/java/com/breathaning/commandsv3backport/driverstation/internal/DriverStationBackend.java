...
public final class DriverStationBackend {
    ...
    private static class HALJoystickTouchpad {
        ...
        final HALJoystickTouchpadFinger[] m_fingers = new HALJoystickTouchpadFinger[MAX_JOYSTICK_TOUCHPAD_FINGERS];
        int m_count;
    ...
    private static class HALJoystickTouchpads {
        ...
        final HALJoystickTouchpad[] m_touchpads = new HALJoystickTouchpad[MAX_JOYSTICK_TOUCHPADS];
        int m_count;
    ...
    private static class MatchDataSender {
        ...
        MatchDataSender() {
            ...
            station.set(1);
            controlWord = table.getIntegerTopic("ControlWord").publish();
            controlWord.set(0);
        }
        ...
        private void sendMatchData() {
            ...
            final int stationNumber = switch (allianceID) {
                case Blue1, Red1 -> 1;
                case Blue2, Red2 -> 2;
                case Blue3, Red3, Unknown -> 3;
            };
            final boolean isRedAlliance = switch (allianceID) {
    ...
    private static class JoystickLogSender {
        ...
        public void send(long timestamp) {
            ...
            if (buttons.m_available != m_prevButtons.m_available || buttons.m_buttons != m_prevButtons.m_buttons) {
        ...
        final HALJoystickAxes m_prevAxes = new HALJoystickAxes(MAX_JOYSTICK_AXES);
        final HALJoystickPOVs m_prevPOVs = new HALJoystickPOVs(MAX_JOYSTICK_POVS);
    ...
    private static HALJoystickTouchpads[] m_joystickTouchpads = new HALJoystickTouchpads[JOYSTICK_PORTS];
    ...
    private static HALJoystickAxesRaw[] m_joystickAxesRawCache = new HALJoystickAxesRaw[JOYSTICK_PORTS];
    private static HALJoystickPOVs[] m_joystickPOVsCache = new HALJoystickPOVs[JOYSTICK_PORTS];
    private static HALJoystickButtons[] m_joystickButtonsCache = new HALJoystickButtons[JOYSTICK_PORTS];
    private static HALJoystickTouchpads[] m_joystickTouchpadsCache = new HALJoystickTouchpads[JOYSTICK_PORTS];
    ...
    static {
        ...
        for (int i = 0; i < JOYSTICK_PORTS; i++) {
            ...
            m_joystickAxes[i] = new HALJoystickAxes(MAX_JOYSTICK_AXES);
            m_joystickAxesRaw[i] = new HALJoystickAxesRaw(MAX_JOYSTICK_AXES);
            m_joystickPOVs[i] = new HALJoystickPOVs(MAX_JOYSTICK_POVS);
            m_joystickTouchpads[i] = new HALJoystickTouchpads();

            m_joystickButtonsCache[i] = new HALJoystickButtons();
            m_joystickAxesCache[i] = new HALJoystickAxes(MAX_JOYSTICK_AXES);
            m_joystickAxesRawCache[i] = new HALJoystickAxesRaw(MAX_JOYSTICK_AXES);
            m_joystickPOVsCache[i] = new HALJoystickPOVs(MAX_JOYSTICK_POVS);
    ...
    public static boolean getStickButton(final int stick, final int button) {
        ...
        reportJoystickWarning(stick, "Joystick Button " + button + " on port " + stick + " not available");
        return false;
    }
    ...
    public static TouchpadFinger getStickTouchpadFinger(int stick, int touchpad, int finger) {
        ...
        if (touchpad < 0 || touchpad >= MAX_JOYSTICK_TOUCHPADS) {
            throw new IllegalArgumentException("Joystick touchpad is out of range");
        }
        if (finger < 0 || finger >= MAX_JOYSTICK_TOUCHPAD_FINGERS) {
            ...
            if (touchpad < touchpadCount) {
                ...
                if (finger < tp.m_count) {
                    return new TouchpadFinger(
                        tp.m_fingers[finger].m_down,
                        tp.m_fingers[finger].m_x,
                        tp.m_fingers[finger].m_y
                    );
                }
            }
        } finally {
        ...
        reportJoystickWarning(
            stick,
            "Joystick touchpad finger " + finger + " on touchpad " + touchpad + " on port " + stick + " not available"
        );
    ...
    public static POVDirection getStickPOV(int stick, int pov) {
        ...
        if (pov < 0 || pov >= MAX_JOYSTICK_POVS) {
    ...
    public static int getJoystickGamepadType(int stick) {
        ...
        return DriverStationJNI.getJoystickIsXbox((byte) stick);
    }
    ...
    public static boolean isAutonomousEnabled() {
        m_cacheDataMutex.lock();
        try {
            return m_controlWord.getAutonomous() && m_controlWord.getEnabled() && m_controlWord.getDSAttached();
        } finally {
            m_cacheDataMutex.unlock();
        }
    }
    ...
    public static boolean isTeleop() {
        return !m_controlWord.getAutonomous() && !m_controlWord.getTest();
    }
    ...
    public static void refreshData() {
        ...
        //             m_joystickTouchpadsCache[stick].m_touchpads[i].m_fingers[j].m_x = m_touchpadFingersCache[
        //                 i * 4 + j * 2 + 0
        //             ];
        //             m_joystickTouchpadsCache[stick].m_touchpads[i].m_fingers[j].m_y = m_touchpadFingersCache[
        //                 i * 4 + j * 2 + 1
        //             ];
        //         }
        //     }
