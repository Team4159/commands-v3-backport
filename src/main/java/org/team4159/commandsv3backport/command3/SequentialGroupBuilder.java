...
public class SequentialGroupBuilder {
    ...
    public SequentialGroupBuilder andThen(Command... nextCommands) {
        ...
        for (int i = 0; i < nextCommands.length; i++) {
            requireNonNullParam(nextCommands[i], "nextCommands[" + i + "]", "SequentialGroupBuilder.andThen");
        }
    ...
    private Command named(String name, boolean useAutomaticName) {
        ...
        var out = new ParallelGroupBuilder().optional(seq, Command.waitUntil(m_endCondition).named("Until Condition"));
