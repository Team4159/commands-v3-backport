...
package com.breathaning.commandsv3backport.driverstation;

import com.breathaning.commandsv3backport.driverstation.internal.DriverStationBackend;
import com.breathaning.commandsv3backport.event.BooleanEvent;
...
public class GenericHID {
    ...
    private final Map<EventLoop, Map<Pair<Integer, Double>, BooleanEvent>> m_axisLessThanCache = new HashMap<>();
    private final Map<EventLoop, Map<Pair<Integer, Double>, BooleanEvent>> m_axisGreaterThanCache = new HashMap<>();
    ...
    public BooleanEvent pov(int pov, POVDirection angle, EventLoop loop) {
        synchronized (m_povCache) {
            ...
            return cache.computeIfAbsent(pov * 16 + angle.value, k ->
                new BooleanEvent(loop, () -> getPOV(pov) == angle)
            );
        }
    }
    ...
    public BooleanEvent axisLessThan(int axis, double threshold, EventLoop loop) {
        synchronized (m_axisLessThanCache) {
            ...
            return cache.computeIfAbsent(Pair.of(axis, threshold), k ->
                new BooleanEvent(loop, () -> getRawAxis(axis) < threshold)
            );
        }
    }
    ...
    public void setRumble(RumbleType type, double value) {
        ...
        DriverStationJNI.setJoystickOutputs((byte) this.m_port, 0, this.m_leftRumble, this.m_rightRumble);
    }
